...
public class RunnerJPanel extends javax.swing.JPanel {
     ...
     */
    @SuppressWarnings({"initialization", "nullable"})
    public RunnerJPanel() {
        initComponents();
        ...
        try {
            if (tableModel instanceof DefaultTableModel) {
                ...
                for (int i = 0; i < colCount && i < argCount; i++) {
                    Object tableValue = jTableArgs.getValueAt(0, i);
                    if (null != tableValue) {
                        data[0][i] = tableValue;
                    } else {
                        data[0][i] = "";
    ...
    public void runProgram() throws IOException {
        ...
        for (int i = 0; i < command.length - 1; i++) {
            String tableValue = (String) jTableArgs.getValueAt(0, i);
            if (null == tableValue) {
                System.err.println("null in table at (" + i + ",0)");
                continue;
            }
            command[i + 1] = tableValue;
            if (null == command[i + 1]) {
                command[i + 1] = "";
            }
        }
