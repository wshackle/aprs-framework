...
//    private final AtomicInteger optionsTableEventsCaused = new AtomicInteger();
    ...
    public ExecutorJPanel(AprsSystem aprsSystem1, Component parentComponent) {
        ...
        optionsTableModelListener = new TableModelListener() {
            @Override
            public void tableChanged(TableModelEvent e) {
                if (e.getType() == TableModelEvent.UPDATE && e.getColumn() == 1) {
                    for (int i = e.getFirstRow(); i <= e.getLastRow(); i++) {
                        int indexView = jTableOptions.convertRowIndexToView(i);
                        Object key = jTableOptions.getValueAt(indexView, 0);
                        if (null == key) {
                            continue;
                        }
                        String keyString = key.toString();
                        if (Objects.equals(keyString, ExecutorOption.ForBoolean.enableOptaPlanner.toString())) {
                            Object bo = jTableOptions.getValueAt(indexView, 1);
                            boolean b;
                            if (bo instanceof Boolean) {
                                b = (Boolean) bo;
                            } else {
                                b = Boolean.parseBoolean(bo.toString());
                            }
                            if (b != enableOptaplannerCachedCheckBox.isSelected()) {
                                enableOptaplannerCachedCheckBox.setSelected(b);
                            }
                            if (b != jCheckBoxEnableOptaPlanner.isSelected()) {
                                jCheckBoxEditOptionsTable.setSelected(b);
                            }
                        }
                    }
                }
    ...
    private final TableModelListener optionsTableModelListener;
    ...
    private @Nullable String selectedToolNameFileName = null;
    ...
    private @Nullable String shortSkuToEmptySlotPrefixMapCsvFileName = null;
     ...
     */
    private @Nullable
    String getShortSkuToEmptySlotPrefixMapCsvFileName() {
        if (null == shortSkuToEmptySlotPrefixMapCsvFileName) {
            try {
                return getDefaulthortSkuToEmptySlotPrefixMapCsvFile();
            } catch (Exception ex) {
                Logger.getLogger(ExecutorJPanel.class.getName()).log(Level.SEVERE, "", ex);
            }
        }
        return shortSkuToEmptySlotPrefixMapCsvFileName;
    }
     ...
     * @param selectedToolNameFileName new value of selectedToolNameFileName
     */
    private void setShortSkuToEmptySlotPrefixMapCsvFileName(String shortSkuToEmptySlotPrefixMapCsvFileName) {
        this.shortSkuToEmptySlotPrefixMapCsvFileName = shortSkuToEmptySlotPrefixMapCsvFileName;
    }
    ...
    private String getDefaulthortSkuToEmptySlotPrefixMapCsvFile() throws IOException {
        return propertiesFile.getCanonicalPath() + ".shortSkuToEmptySlotPrefixMapCsvFile.csv";
    }
...
//            throw new RuntimeException("shouldn't be setting toolname to "+newToolName);
    ...
    private void setOptionsTableEntry(String key, String value) {
        for (int i = 0; i < optionsCachedTable.getRowCount(); i++) {
            ...
            if (Objects.equals(keyFromTable, key) || Objects.equals(keyFromTable.toString(), key)) {
...
//            saveToolHolderContentsMap();
    ...
    private volatile boolean partsToolMapModelListenerEnabled = false;
    
    @UIEffect
    private void setPartsToolMapTableModelListenerOnDisplay() {
        assert SwingUtilities.isEventDispatchThread();
        partsToolMapModelListenerEnabled = true;
        jTableRequiredTools.getModel().addTableModelListener(partsToolMapModelListener);
    }
    
    @UIEffect
    private void clearPartsToolMapTableModelListenerOnDisplay() {
        assert SwingUtilities.isEventDispatchThread();
        partsToolMapModelListenerEnabled = false;
        jTableHolderContents.getModel().removeTableModelListener(partsToolMapModelListener);
    }
    
    @UIType
    @SuppressWarnings("guieffect")
    private class PartsToolMapModelListenerClass implements TableModelListener {
        
        @Override
        public void tableChanged(TableModelEvent e) {
            handlePartsToolMapChange();
        }
        
    }
    
    @UIEffect
    private void handlePartsToolMapChange() {
        if (partsToolMapModelListenerEnabled) {
            Utils.autoResizeTableColWidthsOnDisplay(jTableRequiredTools);
            savePartsToolMap();
        }
    }
    
    private final TableModelListener partsToolMapModelListener = new PartsToolMapModelListenerClass();
    ...
    private void handlePlacePartCompleted(PlacePartInfo ppi) {
        ...
        aprsSystem.logEvent("handlePlacePartCompleted",
                "requestCountDiffer=" + requestCountDiffer + ",aboring=" + aborting + ", ppi.getPddlActionIndex()="
                + ppi.getPddlActionIndex() + ",action=" + ppi.getAction().asPddlLine());
        aprsSystem.logEvent("handlePlacePartCompleted",
                "crclGenerator.getCurrentHeldPart()=" + crclGenerator.getCurrentHeldPart()
                + ",crclGenerator.getPlannedHeldPart()=" + crclGenerator.getPlannedHeldPart());
        ...
        if (requestCountDiffer || aborting) {
            crclGenerator.takeSnapshots("exec",
                    "safeAbortRequested" + sarc + ":" + safeAboutCount.get() + ".ppi=" + ppi, null, null);
            ...
            if (null != parentAction && parentAction.getType() == CHECK_KITS) {
                ...
                appendGenerateAbortLog(logMsg + ".handlePlacePartCompleted.checkKits.abort", actionsListSize,
                        isReverseFlag(), ppi.getParentActionIndex(), sarc, -1);
            } else {
                hppcIndex = ppi.getPddlActionIndex() + 1;
                setReplanFromIndex(hppcIndex, true);
                hppcIndexSet = true;
                appendGenerateAbortLog(logMsg + "handlePlacePartCompleted.abort", actionsListSize, isReverseFlag(),
                        ppi.getPddlActionIndex() + 1, sarc, -1);
            }
        } else {
            appendGenerateAbortLog(logMsg + "handlePlacePartCompleted.continue", actionsListSize, isReverseFlag(),
                    ppi.getPddlActionIndex() + 1, sarc, -1);
        }
    }
        ...
        if (currentActionIndex > 0 && currentActionIndex < pddlOutputCachedTableModel.getRowCount()) {
            ...
            if (o instanceof Integer) {
                ...
                if (crclIndex > 0 && crclIndex < crclProgramCachedTable.getRowCount()) {
                    ...
                    jTableCrclProgram
                            .scrollRectToVisible(new Rectangle(jTableCrclProgram.getCellRect(crclIndex, 0, true)));
                }
            }
        }
    }
    ...
    private void initComponents() {
        ...
        jCheckBoxRequiredToolsEdit = new javax.swing.JCheckBox();
        ...
        jButtonMoveCartesianRecordedPose = new javax.swing.JButton();
        jButtonDeleteRecordedPose = new javax.swing.JButton();
        jButtonMoveRecordedJoints = new javax.swing.JButton();
        ...
        jButtonAddPartToToolEntry.setEnabled(false);
        ...
        jButtonDeletePartToToolEntry.setEnabled(false);
        ...
        jCheckBoxRequiredToolsEdit.setText("Edit");
        jCheckBoxRequiredToolsEdit.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jCheckBoxRequiredToolsEditActionPerformed(evt);
            }
        });
            ...
            .addGroup(jPanelPartToolMapLayout.createSequentialGroup()
                ...
                .addGroup(jPanelPartToolMapLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                    ...
                    .addGroup(jPanelPartToolMapLayout.createSequentialGroup()
                        ...
                        .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
                        .addComponent(jCheckBoxRequiredToolsEdit)
                        .addGap(0, 0, Short.MAX_VALUE)))
                .addContainerGap())
            .addGroup(jPanelPartToolMapLayout.createParallelGroup(javax.swing.GroupLayout.Alignment.LEADING)
                .addGroup(jPanelPartToolMapLayout.createSequentialGroup()
            ...
            .addGroup(javax.swing.GroupLayout.Alignment.TRAILING, jPanel2Layout.createSequentialGroup()
                .addGroup(jPanel2Layout.createParallelGroup(javax.swing.GroupLayout.Alignment.TRAILING)
                    .addGroup(jPanel2Layout.createSequentialGroup()
                        .addContainerGap()
                        .addComponent(positionMapJPanel1, javax.swing.GroupLayout.DEFAULT_SIZE, 837, Short.MAX_VALUE))
                    .addGroup(jPanel2Layout.createSequentialGroup()
                        .addGap(20, 20, 20)
        ...
        jPanel2Layout.setVerticalGroup(
            ...
            .addGroup(jPanel2Layout.createSequentialGroup()
                .addContainerGap()
                .addComponent(positionMapJPanel1, javax.swing.GroupLayout.DEFAULT_SIZE, 264, Short.MAX_VALUE)
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.RELATED)
        ...
        jButtonMoveCartesianRecordedPose.setText("Move Cartesian");
        jButtonMoveCartesianRecordedPose.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButtonMoveCartesianRecordedPoseActionPerformed(evt);
            }
        });

        jButtonDeleteRecordedPose.setText("Delete  Pose");
        jButtonDeleteRecordedPose.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButtonDeleteRecordedPoseActionPerformed(evt);
            }
        });

        jButtonMoveRecordedJoints.setText("Move Joints");
        jButtonMoveRecordedJoints.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jButtonMoveRecordedJointsActionPerformed(evt);
            }
        });
        ...
        jCheckBoxEnableOptaPlanner.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                jCheckBoxEnableOptaPlannerActionPerformed(evt);
            }
        });
    ...
    private void jCheckBoxRequiredToolsEditActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_jCheckBoxRequiredToolsEditActionPerformed
        try {
            final boolean selected = jCheckBoxRequiredToolsEdit.isSelected();
            jButtonDeletePartToToolEntry.setEnabled(selected);
            jButtonAddPartToToolEntry.setEnabled(selected);
            partToolCachedTable.setEditable(selected);
            if(!selected) {
                savePartsToolMap();
            }
    ...
    }//GEN-LAST:event_jCheckBoxRequiredToolsEditActionPerformed
    ...
    private void browseActionsFile() throws IOException {
        ...
        if (null != text) {
            File actionsFile = Utils.file(text);
    ...
    private volatile @Nullable String isDoingActionsInfo = null;
    ...
    public boolean isDoingActions() {
        ...
        if (dasCount > dafCount) {
            String dasTraceInfo = combineDasTraceInfo();
            isDoingActionsInfo = "doingActionsStarted(" + dasCount + ") > doingActionsFinished(" + dafCount + ")"
                    + dasTraceInfo;
            return true;
        } else {
            ...
            if (runningProgram1) {
                ...
                if (continuingActions) {
                    String dasTraceInfo = combineDasTraceInfo();
                    isDoingActionsInfo = "dasCount=" + dasCount
                            + ", isContinuingActions() = true, \n lastContinueActionFuture=" + lastContinueActionFuture
                            + dasTraceInfo;
                    return true;
                } else {
    ...
    private volatile @Nullable String dasIncrementComment = null;
    ...
    public boolean doActions(String comment,
            int startAbortCount,
        ...
        try {
            ...
            if (null != runningProgramFuture) {
                if (!runningProgramFuture.isDone()) {
                    ...
                    throw new IllegalStateException(
                            "starting doActions when runningProgramFuture=" + runningProgramFuture);
                }
            }
            boolean ret = generateCrcl(comment, startAbortCount, options);
            if (ret && atLastAction()) {
            ...
            appendGenerateAbortLog("doActionsReturning comment=" + comment + ",ret=" + ret, actionsListSize, rev,
                    crclGenerator.getLastIndex(), safeAbortRequestCount.get(), -1);
            return ret;
        } catch (Exception ex) {
            System.out.println("");
            System.out.flush();
            ...
            LOGGER.log(Level.SEVERE,
                    "Exception in doActions(" + comment + "," + startAbortCount + ") : " + aprsSystem.getRunName(), ex);
            abortProgram();
    ...
    private void checkReverseWithOptions(ExecutorOption.WithValue<?, ?> @Nullable [] options) {
        ...
        if (!checkReverseDisabled) {
            try {
                ...
                final String errAdd = "ExecutorJPanel.checkReverseWithOptions options = " + Arrays.toString(options);
                LOGGER.log(Level.SEVERE, errAdd, ex);
                throw new RuntimeException(errAdd, ex);
            }
        }
    }
    ...
    public XFuture<Boolean> startActions() {
        if (isDoingActions()) {
            throw new IllegalStateException(
                    "calling startActions when still doing actions isDoingActionsInfo=" + isDoingActionsInfo);
        }
        checkReverse();
        try {
            setReplanFromIndex(0);
            ...
            if (null != runningProgramFuture) {
                if (!runningProgramFuture.isDone()) {
                    runningProgramFuture.cancel(true);
                    throw new IllegalStateException(
                            "calling startActions when runningProgramFuture=" + runningProgramFuture);
                }
            }
    ...
    private void checkReverse() throws IllegalStateException {
        ...
        if (aprsSystem.isReverseFlag() != revFlag) {
            throw new IllegalStateException(
                    "aprsSystem.isReverseFlag() " + aprsSystem.isReverseFlag() + "!= isReverseFlag() " + revFlag);
        }
                ...
                switch (action.getType()) {
                    case TAKE_PART:
                        ...
                        if (partname.contains("_in_pt_") && revFlag) {
                            ...
                            System.err.println(
                                    "resetReadOnlyActionsListReverseFlag = " + resetReadOnlyActionsListReverseFlag);
                            System.err.println("resetReadOnlyActionsListThread = " + resetReadOnlyActionsListThread);
                            System.err.println(
                                    "resetReadOnlyActionsListTrace = " + Arrays.toString(resetReadOnlyActionsListTrace));
                            System.err.println("readOnlyActionsList = " + readOnlyActionsList);
    ...
    private volatile @Nullable Thread resetReadOnlyActionsListThread = null;
...
//    }
    ...
    private @Nullable String actionsFileString = null;
    private @Nullable String reverseActionsFileString = null;
...
//        props.put(MANUAL_SLOT_NAMES, Arrays.toString(getComboSlotNames(10)));
            ...
            props.put(ExecutorOption.ForBoolean.enableOptaPlanner.toString(),
                    Boolean.toString(enableOptaplannerCachedCheckBox.isSelected()));
    ...
    private void updateActionFileStrings(boolean forceNameChange)
            throws HeadlessException, IllegalStateException, IOException {
        if (reverseFlag) {
    ...
    private void setInternalReverseActionsFileString(@Nullable String newReverseActionsFileString,
            boolean forceNameChange) throws IllegalStateException, HeadlessException, IOException {
        if (forceNameChange || reverseActionsFileString == null || reverseActionsFileString.length() < 1) {
            ...
            throw new IllegalStateException("Attempt to change  reverseActionsFileString from "
                    + reverseActionsFileString + " to " + newReverseActionsFileString);
        }
    }
    
    private void checkFilename(@Nullable String filename) throws HeadlessException, IllegalStateException, IOException {
        if (null != filename && filename.length() > 0) {
            File f = Utils.file(filename);
            if (!f.exists()) {
    ...
    public void addAction(Action action) {
        if (null != action) {
            ...
            pddlOutputCachedTableModel.addRow(new Object[]{rowCount, -1, action.getLabel(), action.getType(),
                Arrays.toString(action.getArgs()), finalCost, ""});
        }
    }
...
//        }
//    }
    private static final SolverFactory<OpActionPlan> PRIVATE_SOLVER_FACTORY = OpActionPlan.createSolverFactory();
    
    private @MonotonicNonNull Solver<OpActionPlan> solver = null;
    
    private @MonotonicNonNull Solver<OpActionPlan> showSolver = null;
    ...
    public String readyForNewActionsListInfoString() {
        return "readyForNewActionsListInfoString:\n"
                ...
                + ((null != crclGenerator)
                        ? "crclGenerator.getLastIndex()=" + crclGenerator.getLastIndex()
                        + ", crclGenerator.atLastIndex()=" + crclGenerator.atLastIndex()
                        : "crclGenerator=null");
    }
    ...
    public List<Action> loadActionsFile(
        ...
        if (f.isDirectory()) {
            throw new IllegalArgumentException(
                    "Can not loadActionsFile \"" + f + "\" : it is a directory instead of a text file.");
        }
        if (!f.canRead()) {
...
//                        startIndex = i + 1;
            ...
            if (null != bestPlan && null != worstPlan) {
                ...
                aprsSystem.runOnDispatchThread(() -> showLoadedPlanOptaPlannerOnDisplay(worstPlanFinal, worstScoreFinal,
                        bestPlanFinal, bestScoreFinal));
            }
        }
    }
    
    @UIEffect
    private void showLoadedPlanOptaPlannerOnDisplay(OpActionPlan worstPlan, double worstScore, OpActionPlan bestPlan,
            double bestScore) {
        if (firstLoad) {
    ...
    private void jButtonLoadPddlActionsFromFileActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButtonLoadPddlActionsFromFileActionPerformed
        try {
            ...
            browseActionsFile();
            loadActionsFile(
                    Utils.file(pddlOutputActionsCachedText.getText()), // File f,
                    true, // boolean showInOptaPlanner,
                    reverseFlag, // newReverseFlag
        ...
        try {
            ...
            if (pddlOutputActionsCachedText.getText().isEmpty()) {
                JOptionPane.showMessageDialog(parentComponent,
                        "pddlOutputActionsCachedText is empty or blank, enter filename in field or use browse button.");
                return;
            }
            loadActionsFile(
                    Utils.file(pddlOutputActionsCachedText.getText()), // File f,
    ...
    private boolean checkResetReverseActionsFile() throws HeadlessException {
        if (reverseFlag && null != reverseActionsFileString && reverseActionsFileString.length() > 0) {
            int confirm = JOptionPane.showConfirmDialog(this,
                    "Reset reverseActionsFileString=" + reverseActionsFileString);
    ...
    private @MonotonicNonNull Callable<DbSetupPublisher> dbSetupSupplier = null;
    ...
    private CRCLProgramType createEmptyProgram() {
        ...
        if (trace.length > 3) {
            ...
            program.setName("empty" + emptyProgramCount.incrementAndGet() + "_" + fileNameShort + ":"
...
//        setCommandId(endCmd);
    ...
    private volatile @Nullable CRCLProgramType unstartedProgram = null;
    ...
    private String getActionsCrclName() {
        ...
        return toNonColonizedName(actionsName + ":" + currentActionIndex + "_" + crclGenerator.getLastIndex() + ":"
                + crclGenerator.getCrclNumber());
    }
    ...
    private boolean runCrclProgram(CRCLProgramType crclProgram) {
        try {
            ...
            try {
                System.err.println(
                        "crclProgram :" + CRCLSocket.getUtilSocket().programToPrettyString(crclProgram, false));
            } catch (Exception ex1) {
    ...
    private void jButtonPddlOutputViewEditActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButtonPddlOutputViewEditActionPerformed
        try {
            Desktop.getDesktop().open(Utils.file(pddlOutputActionsCachedText.getText()));
    ...
    private @Nullable String currentPart = null;
    ...
    private volatile @Nullable Thread setReplanFromIndexLastThread = null;
    private volatile @Nullable Thread prevSetReplanFromIndexLastThread = null;
    ...
    private void setReplanFromIndex(int replanFromIndex, boolean aborting) {
        ...
        if (oldRpi != replanFromIndex) {
            ...
            appendGenerateAbortLog("setReplanFromIndex(" + replanFromIndex + "," + aborting + ") oldRpi=" + oldRpi,
                    actionsListSize, reverseFlag, replanFromIndex, safeAbortRequestCount.get(), -1);
            if (!aborting && oldRpi > replanFromIndex) {
                ...
                if (replanFromIndex != 0 || !readyForNewActionsList()) {
                    ...
                    logDebug("Reducing replanFromIndex when generater not readyForNewActionsList: oldRpi=" + oldRpi
                            + ", new replanFromIndex=" + replanFromIndex
                            + ",  pddlActionToCrclGenerator.getLastIndex()=" + crclGenerator.getLastIndex());
                }
            }
            @Nullable
            ...
            aprsSystem.runOnDispatchThread(() -> {
                ...
                jTablePddlOutput
                        .scrollRectToVisible(new Rectangle(jTablePddlOutput.getCellRect(replanFromIndex, 0, true)));
                if (replanFromIndex == 0) {
                    jTextFieldCurrentPart.setText("");
    ...
    private void clearAll() {
        if (isDoingActions()) {
            throw new IllegalStateException(
                    "calling clearAll when still doing actions isDoingActionsInfo=" + isDoingActionsInfo);
        }
        warnIfNewActionsNotReady();
    ...
    private volatile @Nullable Thread abortProgramThread = null;
    private volatile StackTraceElement abortProgramTrace@Nullable []  = null;
    private volatile @Nullable XFutureVoid abortProgramFuture = null;
    private volatile @Nullable XFutureVoid abortProgramAbortCrclFuture = null;
    ...
    public XFutureVoid abortProgram() {
        ...
        appendGenerateAbortLog("start_abortProgram", actionsListSize, isReverseFlag(), replanFromIndex.get(),
    ...
    private volatile @Nullable Thread completeAbortProgramThread = null;
    ...
    private synchronized void completeAbortProgram(boolean rps) {
        ...
        appendGenerateAbortLog("complete_abortProgram", actionsListSize, isReverseFlag(), replanFromIndex.get(),
                safeAbortRequestCount.get(), -1);
    }
        ...
        if (!cancelSafeAbortFutures) {
            try {
                if (crclGenerator.isTakeSnapshots() && aprsSystem.snapshotsEnabled()) {
                    takeSimViewSnapshot(aprsSystem.createTempFile("-safe-abort-", ".PNG", aprsSystem.getLogImageDir()),
                            null, "");
                }
...
//            this.jTextFieldReturnCount.setText(Integer.toString(returnCount));
        ...
        try {
            ...
            if (null != partName && partName.length() > 0) {
                ...
                if (null != poseFromDb) {
                    ...
                    String offsetString = (testDropOffPosePoint.getX() - poseFromDbPoint.getX())
                    ...
                    writeCorrectionCsv(recordCsvName,
                            System.currentTimeMillis() + ", " + partName + ", " + randomPoseString + ", "
                            + poseFromDbString + ", " + offsetString);
                }
    ...
    /*
     * private XFuture<Boolean> startRandomTest() { clearAll(); queryLogFileName();
     * jCheckBoxReplan.setSelected(true); autoStart = true;
     *
     * PointType testDropOffPosePoint = requireNonNull(testDropOffPose.getPoint(),
     * "testDropOffPose.getPoint()"); String randomPoseString =
     * String.format("%.1f, %.1f, %.1f", testDropOffPosePoint.getX(),
     * testDropOffPosePoint.getY(), testDropOffPosePoint.getZ());
     * logDebug("randomPoseString = " + randomPoseString);
     * logDebug("randomDropOffCount = " + randomDropOffCount);
     * customRunnables.clear(); customRunnables.add(() -> {
     * logDebug("Continuing with lookFor"); this.lookForParts(); });
     * customRunnables.add(() -> {
     * logDebug("Continuing with recordAndCompletRandomPickup");
     * this.recordAndCompletTestPickup(); }); customRunnables.add(() -> {
     * logDebug("Continuing with randomDropOff"); this.randomDropOff(); });
     * this.customRunnablesIndex = 0; this.replanRunnable =
     * this.customReplanRunnable; return this.randomDropOff()
     * .thenCompose("randomTest.lookForParts", x -> recursiveSupplyBoolean(x,
     * this::lookForParts)) .thenCompose("randomTest.recordAndCompletTestPickup", x
     * -> recursiveSupplyBoolean(x, this::recordAndCompletTestPickup))
     * .thenCompose("randomTest.randomDropOff", x -> recursiveSupplyBoolean(x,
     * this::randomDropOff)); }
     */
    @UIEffect
...
//            jCheckBoxAutoStartCrcl.setSelected(true);
            ...
            this.gridDropOff();
            PointType testDropOffPosePoint = requireNonNull(testDropOffPose.getPoint(), "testDropOffPose.getPoint()");
            String randomPoseString = String.format("%.1f, %.1f, %.1f",
...
//        this.jTextFieldReturnCount.setText("0");
    ...
    private void jButtonRecordFailActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButtonRecordFailActionPerformed
        ...
        try {
            File f = Utils.file(jTextFieldLogFilename.getText());
...
//            setRunProgramFuture(generateCrclAsync());
    ...
    private volatile @Nullable XFuture<Boolean> runningProgramFuture = null;
    ...
    private void incSafeAbortCount(String label) {
        ...
        appendGenerateAbortLog(label + ".incSafeAbortCount", actionsListSize, isReverseFlag(), replanFromIndex.get(),
                count, -1);
    }
    ...
    private void incSafeAbortRequestCount() {
        ...
        appendGenerateAbortLog("incSafeAbortRequestCount", actionsListSize, isReverseFlag(), replanFromIndex.get(),
                count, -1);
...
//    private volatile long lastCheckSafeAbortTime = 0;
        ...
        logDebug("(curTime - runProgramCompleteRunnablesTime)           = "
                + (curTime - runProgramCompleteRunnablesTime));
    ...
    private volatile @Nullable XFutureVoid lastSafeAbortFuture = null;
    private volatile @Nullable XFuture<Boolean> startSafeAbortRunningProgramFuture = null;
    private volatile boolean startSafeAbortRunningProgramFutureDone = false;
    
    private volatile @Nullable CRCLProgramType startSafeAbortProgram = null;
    private volatile @Nullable String startSafeAbortProgramName = null;
    ...
    public XFutureVoid startSafeAbort(String name) {
        ...
        synchronized (this) {
            ...
            startSafeAbortRunningProgramFutureDone = startSafeAbortRunningProgramFuture != null
            ...
            if (!startSafeAbortRunningProgram) {
                ...
                return XFutureVoid.completedFutureWithName(
                        "!startSafeAbortRunningProgram" + startSafeAbortRequestCount + ":" + safeAboutCount.get() + ":"
                        + name + ":pddlExecutorStartSafeAbort." + aprsSystem.getRunName());
            }
            if (startSafeAbortRunningProgramFutureDone) {
                incSafeAbortCount("startSafeAbort.startSafeAbortRunningProgramFutureDone." + name);
                return XFutureVoid.completedFutureWithName("startSafeAbortRunningProgramFutureDone"
                        + startSafeAbortRequestCount + ":" + safeAboutCount.get() + ":" + name
                        + ":pddlExecutorStartSafeAbort." + aprsSystem.getRunName());
            }
            if (!startSafeAbortIsRunningCrclProgram) {
                incSafeAbortCount("startSafeAbort!startSafeAbortIsRunningCrclProgram." + name);
                return XFutureVoid.completedFutureWithName(
                        "!startSafeAbortIsRunningCrclProgram" + startSafeAbortRequestCount + ":" + safeAboutCount.get()
                        + ":" + name + ":pddlExecutorStartSafeAbort." + aprsSystem.getRunName());
            }
            
            final XFutureVoid ret = new XFutureVoid(startSafeAbortRequestCount + ":" + safeAboutCount.get() + ":" + name
                    + ":pddlExecutorStartSafeAbort." + aprsSystem.getRunName());
    ...
    private volatile @Nullable XFutureVoid lastContinueActionFuture = null;
    ...
    boolean completeActionList(String comment, int startSafeAbortRequestCount, StackTraceElement[] callerTrace) {
        try {
            checkReverse();
            boolean rev = isReverseFlag();
            appendGenerateAbortLog("completeActionListStarting" + comment, actionsListSize, rev, getReplanFromIndex(),
                    startSafeAbortRequestCount, -1);
            ...
            dasIncrementComment = comment;
            autoStart = true;
            boolean ret = generateCrcl(comment, startSafeAbortRequestCount, null);
            if (ret && atLastAction()) {
                actionSetsCompleted.set(actionSetsStarted.get());
            }
            if (ret) {
            ...
            appendGenerateAbortLog("completeActionListReturning." + ret, actionsListSize, rev, getReplanFromIndex(),
                    safeAbortRequestCount.get(), -1);
            return ret;
        } catch (Exception ex) {
            LOGGER.log(Level.SEVERE, "", ex);
            ...
            System.err.println(
                    "prevSetReplanFromIndexLastTrace = " + Utils.traceToString(prevSetReplanFromIndexLastTrace));
                ...
                if (null != poseFromDb) {
                    ...
                    newRobotStatusFuture
                            .thenAccept((CRCLStatusType newStatus) -> {
                                try {
                                    PoseStatusType poseStatus = requireNonNull(newStatus.getPoseStatus(),
                                            "aprsSystem.getPoseStatus()");
                                    ...
                                    String curPoseString = String.format("%.1f, %.1f, %.1f",
                                            curPosePoint.getX(),
                                            curPosePoint.getY(),
                                            curPosePoint.getZ());
                                    System.out
                                            .println("jButtonRecordActionPerformed: curPoseString = " + curPoseString);
                                    String offsetString = (curPosePoint.getX() - poseFromDbPoint.getX())
                                    ...
                                    final String csvLine = System.currentTimeMillis() + ", " + partName + ", "
                                            + curPoseString + ", " + poseFromDbString + ", " + offsetString;
    ...
    private void updateLookForJoints(CRCLStatusType stat) {
        if (null != stat) {
            ...
            if (null != jointStatuses) {
                ...
                String jointVals = jointStatusListToString(jointListIterable);
                logDebug("jointVals = " + jointVals);
                boolean keyFound = false;
                for (int i = 0; i < optionsCachedTable.getRowCount(); i++) {
                    final Object keyi = optionsCachedTable.getValueAt(i, 0);
                    if (Objects.equals(ExecutorOption.ForString.lookForJoints, keyi)
                            || Objects.equals(ExecutorOption.ForString.lookForJoints.toString(), keyi.toString())) {
//                        optionsTableEventsCaused.incrementAndGet();
                ...
                if (!keyFound) {
                    optionsCachedTable.addRow(new Object[]{ExecutorOption.ForString.lookForJoints.toString(), jointVals});
                }
                crclGenerator.setOptions(getOptions());
            }
            if (null != poseStatus) {
                ...
                for (int i = 0; i < optionsCachedTable.getRowCount(); i++) {
                    final Object keyi = optionsCachedTable.getValueAt(i, 0);
                    if (Objects.equals(ExecutorOption.ForString.lookForXYZ, keyi)
                            || Objects.equals(ExecutorOption.ForString.lookForXYZ.toString(), keyi.toString())) {
//                        optionsTableEventsCaused.incrementAndGet();
    ...
    private String jointStatusListToString(Iterable<JointStatusType> jointStatusIterable) {
        String jointVals = StreamSupport.stream(jointStatusIterable.spliterator(), false)
    ...
    private void loadPartToolMap() throws IOException {
        ...
        if (!f.exists()) {
            setPartsToolMapTableModelListenerOnDisplay();
            clearEmptyPartToolRows();
            return;
        }
        clearPartsToolMapTableModelListenerOnDisplay();
        int lineNumber = 0;
    ...
    private void loadToolChangerPoseMap() throws IOException {
        ...
        Map<String, PoseType> tablePoseMap = crclGenerator.getToolHolderPoseMap();
        final ConcurrentMap<String, String> jointsMap = crclGenerator.getToolChangerJointValsMap();
        loadPosesFileToTable(table, f, tablePoseMap, jointsMap, true);
    }
    ...
    private void loadRecordedPosesMap() throws IOException {
        ...
        int lineNumber = 0;
        crclGenerator.getRecordedPosesJointValsMap().clear();
        final CachedTable table = recordedPositionsCachedTable;
        Map<String, PoseType> tablePoseMap = crclGenerator.getRecordedPoseMap();
        final ConcurrentMap<String, String> jointsMap = crclGenerator.getRecordedPosesJointValsMap();
        loadPosesFileToTable(table, f, tablePoseMap, jointsMap, false);
    }
    
    private void loadPosesFileToTable(final CachedTable table, File f, Map<String, PoseType> tablePoseMap,
        ...
        try ( CSVParser parser = new CSVParser(new FileReader(f), CSVFormat.DEFAULT.withFirstRecordAsHeader())) {
            ...
            for (int i = skipRows; i < records.size(); i++) {
                ...
                try {
                    ...
                    if (null != jointVals && jointVals.length() > 0 && (approach || !toolHolderMode)) {
                        jointsMap.put(name, jointVals);
                    }
                    if (!approach || !toolHolderMode) {
    ...
    public File getToolChangerPoseFile() throws IOException {
        ...
        if (textFieldPath != null && textFieldPath.trim().length() > 0) {
            File f = Utils.file(textFieldPath);
            if (f.exists() && f.canRead()) {
        ...
        String optionPath = crclGenerator.getStringOptionsMap().get(ExecutorOption.ForString.toolChangerPoseFile);
    ...
    public File getRecordedPoseFile() throws IOException {
        ...
        String optionPath = crclGenerator.getStringOptionsMap().get(ExecutorOption.ForString.recordedPositionsFile);
        if (optionPath != null && optionPath.trim().length() > 0) {
            File f = Utils.file(optionPath);
            if (f.exists() && f.canRead()) {
                final String canonicalPath = f.getCanonicalPath();
    ...
    public File getPartToolFile() throws IOException {
        ...
        String optionPath = crclGenerator.getStringOptionsMap().get(ExecutorOption.ForString.partToolFile);
...
//    }
    ...
    private void savePartsToolMap() {
        try {
            if (null == propertiesFile || !propertiesFile.exists() || partToolCachedTable.getRowCount() < 1) {
                return;
            }
            File f = getPartToolFile();
            if (null != f) {
                Utils.saveCachedTable(f, partToolCachedTable);
            }
        } catch (IOException ex) {
            LOGGER.log(Level.SEVERE, "", ex);
        }
    ...
    @UIEffect
    @SuppressWarnings("nullness")
    private String queryUserForNewRecordedPosName(String qname) {
        ...
        for (int i = 0; i < oldNames.length; i++) {
            ...
            if (oldName.equals(queryResult)) {
                if (JOptionPane.YES_OPTION == JOptionPane.showConfirmDialog(this,
                        queryResult + " already in table. Overwrite?")) {
                    return queryResult;
                } else {
                    return queryUserForNewRecordedPosName(qname);
                }
            }
        }
        return queryResult;
    ...
    @UIEffect
    @SuppressWarnings("nullness")
    private String queryUserForExistingRecordedPosName(String qname) {
        return (String) JOptionPane.showInputDialog(
                ...
                JOptionPane.QUESTION_MESSAGE, // messageType
                (Icon) null, // icon
                getRecordedPoseNames(), // selectionValues
...
//        }
        for (int i = 0; i < table.getRowCount(); i++) {
            ...
            if (null != entryName && (entryApproachObject instanceof Boolean)) {
                ...
                logDebug("Bad table entry: " + i + ", entryName=" + entryName + ", entryApproachObject="
                        + entryApproachObject);
            }
        }
        return -1;
...
//        CRCLStatusType stat = aprsSystem.getCurrentStatus();
        if (null != stat) {
            JointStatusesType jointStatuses = stat.getJointStatuses();
            if (null != jointStatuses) {
                Iterable<JointStatusType> jointList = CRCLUtils.getNonNullJointStatusIterable(jointStatuses);
                String jointVals = jointStatusListToString(jointList);
                return jointVals;
            }
        }
        return null;
...
//    }
    ...
    private void updateToolChangePose(String name, boolean approach, PoseType pose, PmRpy rpy,
            @Nullable String jointString) {
...
//            toolChangerPose = pose;
    ...
    private void renameFromToolHolderContentsTable(String oldName, String newName) {
        Map<String, String> expectedToolHolderContentsMap = crclGenerator.getExpectedToolHolderContentsMap();
        String oldExpectedContents = expectedToolHolderContentsMap.get(oldName);
        Map<String, String> currentToolHolderContentsMap = crclGenerator.getCurrentToolHolderContentsMap();
    ...
    private XFutureVoid updatePoseCacheOnDisplay(int startAbortCount) {
        try {
            final ExecutorService generateCrclService = aprsSystem.getRunProgramService();
            XFuture<List<PhysicalItem>> newItemsFuture = XFuture.supplyAsync("userRequestedPoseUpdate",
                    () -> {
                        try {
                            crclGenerator.clearPoseCache();
...
//                        },generateCrclService);
    ...
    private void jButtonRenameToolHolderPoseActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButtonRenameToolHolderPoseActionPerformed
        ...
        String newName = JOptionPane.showInputDialog(
                this, // parentComponent
                "Tool Holder Pose Name?", // Object message
                aprsSystem.getTaskName() + " " + aprsSystem.getRobotName() + " new holder name for " + oldName, // String
                // title
                JOptionPane.QUESTION_MESSAGE // messageType
        );
        ...
        try {
            ...
            if (null != partName && partName.length() > 0) {
                try {
                    ...
                    final ExecutorService generateCrclService = aprsSystem.getRunProgramService();
                    XFuture<?> quickCalibFuture = XFuture.supplyAsync("userRequestedPoseUpdate",
                            () -> {
                                try {
                                    crclGenerator.clearPoseCache();
                                    return lookForParts();
    ...
    public void storeNamedPoseAndJointString(String recordedPoseName, PoseType pose, String jointString)
            throws PmException {
        Map<String, PoseType> recordedPoseMap = crclGenerator.getRecordedPoseMap();
    ...
    private void jButtonMoveCartesianRecordedPoseActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButtonMoveCartesianRecordedPoseActionPerformed
        try {
            ...
            abortProgram();
            autoStart = true;
            String recordedPoseName = queryUserForExistingRecordedPosName("Move Cartesian Recorded Pose");
            if (null == recordedPoseName
                    || recordedPoseName.length() < 1) {
    ...
    private void jButtonDeleteRecordedPoseActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButtonDeleteRecordedPoseActionPerformed
        try {
            Map<String, PoseType> recordedPoseMap = crclGenerator.getRecordedPoseMap();
            String nameToDelete = queryUserForExistingRecordedPosName("Record Pose");
            if (null == nameToDelete
                    || nameToDelete.length() < 1) {
                return;
            }
            if (!recordedPoseMap.keySet().contains(nameToDelete)) {
                JOptionPane.showMessageDialog(this,
                        "nameToDelete=" + nameToDelete + " is not in " + recordedPoseMap.keySet());
                return;
            }
//            toolChangerPose = pose;
            recordedPoseMap.remove(nameToDelete);
            crclGenerator.getRecordedPosesJointValsMap().remove(nameToDelete);
    ...
    private void jButtonMoveRecordedJointsActionPerformed(java.awt.event.ActionEvent evt) {// GEN-FIRST:event_jButtonMoveRecordedJointsActionPerformed
        try {
            ...
            abortProgram();
            autoStart = true;
            String recordedJointsName = queryUserForExistingRecordedPosName("Move Recorded Joints");
            if (null == recordedJointsName
                    || recordedJointsName.length() < 1) {
    ...
    private XFuture<Boolean> cartesianMoveToRecordedPosition(String recordedPoseName, boolean manualAction) {
        ...
        if (null != generateCrclService) {
            return XFuture.supplyAsync("cartesianMoveToRecordedPosition:" + recordedPoseName,
                    () -> {
                        try {
                            XFuture<Boolean> future = this.moveToRecordedPose(recordedPoseName);
    ...
    private XFuture<Boolean> jointMoveToNamedPosition(String recordedJointsName, boolean manualAction) {
        ...
        if (null != generateCrclService) {
            return XFuture.supplyAsync("jointMoveToNamedPosition:" + recordedJointsName,
                    () -> {
                        try {
                            XFuture<Boolean> future = this.moveToRecordedJoints(recordedJointsName, manualAction);
                            setRunProgramFuture(future);
                            return future;
                        } catch (Exception ex) {
                            Logger.getLogger(ExecutorJPanel.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public XFuture<Boolean> openGripper() {
        ...
        if (null != generateCrclService) {
            return XFuture.supplyAsync("openGripper",
                    () -> {
                        try {
                            XFuture<Boolean> future = this.openGripperInternal();
                            setRunProgramFuture(future);
                            return future;
                        } catch (Exception ex) {
                            Logger.getLogger(ExecutorJPanel.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public XFuture<Boolean> closeGripper() {
        ...
        if (null != generateCrclService) {
            return XFuture.supplyAsync("closeGripper",
                    () -> {
                        try {
                            XFuture<Boolean> future = this.closeGripperInternal();
                            setRunProgramFuture(future);
                            return future;
                        } catch (Exception ex) {
                            Logger.getLogger(ExecutorJPanel.class.getName()).log(Level.SEVERE, null, ex);
...
//            });
    ...
    private volatile @Nullable PointType lastSelectedPoseCachePoint = null;
    
    private volatile @Nullable String lastSelectedPoseCachePointPartName = null;
    
    private volatile @Nullable PointType lastSelectedRobotPoint = null;
    
    private volatile @Nullable PointType lastSelectedPoseCacheRobotOffset = null;
    ...
    private void updateSelectedPartPoseInfo() {
        ...
        if (null != pose) {
            PointType point = pose.getPoint();
            if (null != point) {
                jTextFieldErrMapPartInfo
                        .setText(String.format("%s : %.3f,%.3f,%.3f", part, point.getX(), point.getY(), point.getZ()));
                ...
                jTextFieldAdjPose.setText(
                        String.format("%.1f,%.1f,%.1f", robotPoint.getX(), robotPoint.getY(), robotPoint.getZ()));
                PointType offsetPoint = CRCLPosemath.point(point.getX() - robotPoint.getX(),
                        point.getY() - robotPoint.getY(), point.getZ() - robotPoint.getZ());
                jTextFieldOffset.setText(
                        String.format("%.1f,%.1f,%.1f", offsetPoint.getX(), offsetPoint.getY(), offsetPoint.getZ()));
    ...
    private void completeQuickCalib(String partName, PointType curPosePoint, String curPoseString) {
        try {
            ...
            if (null != poseFromDb) {
                ...
                String visionPointString = visionPoint.getX()
                        + "," + visionPoint.getY()
                        + "," + curPosePoint.getZ();
                ...
                String offsetString = (curPosePoint.getX() - visionPoint.getX())
                        + "," + (curPosePoint.getY() - visionPoint.getY())
                        + "," + (0);
                logDebug("offsetString = " + offsetString);
                aprsSystem.logEvent("jButtonQuickCalibActionPerformed: offsetString", offsetString);
                final String csvLine = System.currentTimeMillis() + ", " + partName + ", " + visionPointString + ", "
                        + curPoseString + ", " + offsetString;
                writeCorrectionCsv(recordCsvName, csvLine);
    ...
    @UIEffect
    private void queryLogFileName() throws IOException {
        File recordCsvFile = Utils.file(recordCsvName);
        if (!recordCsvFile.exists()) {
            newLogFileName();
        }
    }
...
//        }
    ...
    private @Nullable String crclProgName = null;
    private @Nullable String lastCrclProgName = null;
    ...
    /*
     * @SafeEffect private XFuture<Boolean> recursiveSupplyBoolean(boolean
     * prevSuccess, UiSupplier<XFuture<Boolean>> supplier) { if (prevSuccess) { try
     * { return Utils.composeOnDispatchThread(supplier); } catch (Exception ex) {
     * LOGGER.log(Level.SEVERE, "", ex); XFuture<Boolean> ret = new
     * XFuture<>("recursiveSupplyBoolean"); ret.completeExceptionally(ex); return
     * ret; } } else { return XFuture.completedFuture(false); } }
     */
    ...
    private volatile @Nullable String lastCheckAbortCurrentPart = null;
    private volatile boolean lastCheckAbortSafeAbortRequested = false;
    private volatile long lastCheckSafeAbortTime = 0;
    ...
    private boolean checkSafeAbort(int startSafeAbortRequestCount) {
        ...
        synchronized (this) {
            ...
            if (initialSafeAbortRequested != aprsSystem.isAborting()) {
                logDebug("safeAbortRequested=" + initialSafeAbortRequested + ", aprsSystemInterface.isAborting()="
                        + aprsSystem.isAborting());
    ...
    private boolean generateCrcl(
        ...
        if (null != options) {
            origOptionsMap = this.getOptions();
        ...
        try {
            ...
            while (replanAfterCrclBlock && autoStart) {
                ...
                replanAfterCrclBlock = nextReplanAfterCrclBlock;
                lastReplanAfterCrclBlock = replanAfterCrclBlock;
            }
...
//        }
    ...
    private XFuture<Boolean> generateCrclAsync() throws IllegalStateException {
        ...
        final ExecutorService generateCrclService = aprsSystem.getRunProgramService();
        String taskName = "generateCrcl(" + aprsSystem.getTaskName() + ").doPddlActionsSection("
                + crclGenerator.getLastIndex() + " out of " + actionsListSize + ")";
    ...
    private volatile @Nullable File generateAbortLogFile = null;
    ...
    private XFutureVoid appendGenerateAbortLog(String type, int actionsSize, boolean reverse, int startingIndex,
        ...
        try {
            ...
            Object[] rowValues = new Object[]{type, reverse, actionsSize, startingIndex, startSafeAbortRequestCount,
                sectionNumber, aprsSystem.getRunNumber(), initialRunningProgram, aprsSystem.getRobotName(),
                actionsFileName};
...
//            println("generateAbortLogFile = " + generateAbortLogFile);
    ...
    private synchronized List<MiddleCommandType> generate(
            ...
            throws Exception {
        appendGenerateAbortLog("generate", actions.size(), isReverseFlag(), startingIndex, startSafeAbortRequestCount,
                sectionNumber);
        List<MiddleCommandType> ret = crclGenerator.generate(actions, startingIndex, options,
                startSafeAbortRequestCount, manualAction);
    ...
    @UIEffect
    private void updatePddlActionSectionToCrclDisplay(int[] indexesCopy, String[] labelsCopy,
            @Nullable String[] takenPartNamesCopy) {
    ...
    private void setEndCanonCmdId(CRCLProgramType program) {
        ...
        if (endCmdId != expectedEndCmdId) {
            logDebug("EndCanon Id " + endCmdId + " doesn't match InitCanon id " + initCmdId
                    + " + 1+ size of middle commands " + midSize);
        }
    }
            ...
            if (null != pose) {
                ...
                if (null != point) {
                    ...
                    try {
                        ...
                        positionCacheCachedTable
                                .addRow(new Object[]{entry.getKey(), point.getX(), point.getY(), point.getZ(),
                            Math.toDegrees(rpy.r), Math.toDegrees(rpy.p), Math.toDegrees(rpy.y), ""});
                    } catch (PmException ex) {
                        positionCacheCachedTable.addRow(new Object[]{entry.getKey(), point.getX(), point.getY(),
                            point.getZ(), Double.NaN, Double.NaN, Double.NaN, ex.toString()});
                        Logger.getLogger(ExecutorJPanel.class.getName()).log(Level.SEVERE, "", ex);
                    }
                }
            }
    ...
    private XFuture<Boolean> doPddlActionsSectionAsync(int startSafeAbortRequestCount, int sectionNumber) {
        try {
            ...
            if (autoStart) {
                boolean replanAfterCrclBlock = crclGenerator.getLastIndex() < actionsListSize - 1
                        && isReplanCheckBoxSelected();
                lastReplanAfterCrclBlock = replanAfterCrclBlock;
                if (replanAfterCrclBlock) {
                    return startCrclProgram(program)
                            .thenCompose(
                                    "doPddlActionsSection.recursiveApplyGenerateCrcl(" + crclGenerator.getLastIndex()
                                    + " out of " + actionsListSize + ")",
    ...
    private void updatePoseTextFields() {
        for (PositionMap positionMap : getPositionMaps()) {
            if (null != positionMap) {
                ...
                if (null != testPoint) {
                    String testPoseString = String.format("%.1f, %.1f, %.1f",
                            testPoint.getX(),
                            testPoint.getY(),
                            testPoint.getZ());
                ...
                if (null != origPoint) {
                    String origPoseString = String.format("%.1f, %.1f, %.1f",
                            origPoint.getX(),
                            origPoint.getY(),
                            origPoint.getZ());
    ...
    private XFuture<Boolean> moveToRecordedPose(String recordedPoseName, boolean manualAction) throws Exception {
        ...
        actionsList.add(moveRecordedPoseAction);
        syncCrclGeneratorPositionMaps();
        CRCLProgramType program = createEmptyProgram();
        Solver<OpActionPlan> origSolver = crclGenerator.getSolver();
        crclGenerator.setSolver(null);
    ...
    private XFuture<Boolean> moveToRecordedJoints(String recordedName, boolean manualAction) throws Exception {
        ...
        actionsList.add(moveRecordedJointsAction);
        syncCrclGeneratorPositionMaps();
        CRCLProgramType program = createEmptyProgram();
        Solver<OpActionPlan> origSolver = crclGenerator.getSolver();
        crclGenerator.setSolver(null);
    ...
    private void syncCrclGeneratorPositionMaps() {
        if (isRunningProgram()) {
            ...
            setDebug(origDebug);
            throw new IllegalStateException(
                    "Attempting to change position maps when program running. runningProgramFuture="
                    + runningProgramFuture);
        }
    ...
    private XFuture<Boolean> randomDropOff() {
        ...
        String randomPoseString = String.format("%.1f, %.1f, %.1f",
                ...
                testDropOffPosePoint.getY(),
                testDropOffPosePoint.getZ());
        PointType origPosePoint = requireNonNull(origPose.getPoint(), "origPose.getPoint()");
        String origPoseString = String.format("%.1f, %.1f, %.1f",
                origPosePoint.getX(),
                origPosePoint.getY(),
                origPosePoint.getZ());
    ...
    private void gridDropOff() {
        ...
        setCrclProgram(program);
        PointType testDropOffPosePoint = requireNonNull(testDropOffPose.getPoint(), "testDropOffPose.getPoint()");
        String gridPoseString = String.format("%.1f, %.1f, %.1f",
                testDropOffPosePoint.getX(),
                testDropOffPosePoint.getY(),
                testDropOffPosePoint.getZ());
        PointType origPosePoint = requireNonNull(origPose.getPoint(), "origPose.getPoint()");
    ...
    private XFuture<Boolean> gotoToolChangerApproach(String poseName, PoseType pose) {
        try {
            ...
            crclGenerator
                    .setApproachToolChangerZOffset(Double.parseDouble(jTextFieldToolChangerApproachZOffset.getText()));
            setOptionsTableEntry("approachToolChangerZOffset", jTextFieldToolChangerApproachZOffset.getText());
            ...
            for (int i = 0; i < holderContentsCachedTable.getRowCount(); i++) {
                ...
                if (toolChangerPosName.equals(holderName)) {
                    ...
                    if (!toolName.equals(origToolName)) {
                        ...
                        holderContentsCachedTable
                                .setValueAt("contents changed from " + origToolName + " on " + new Date(), i, 3);
    ...
    private XFuture<Boolean> executeActionsInternal(List<Action> actionsList, Map<ExecutorOption, ?> options) {
        try {
            CRCLProgramType program = createEmptyProgram();
            List<MiddleCommandType> cmds = generate(actionsList,
                    0,
                    options,
    ...
    private XFuture<Boolean> pickupToolByHolder(String holderName) {
        try {
            ...
            syncPanelToGeneratorToolDataOnDisplay();
            List<Action> newActionsList = new ArrayList<>();
            Action pickupToolByHolderAction = Action.newSingleArgAction(
    ...
    private XFuture<Boolean> pickupToolByTool(String toolName) {
        try {
            ...
            syncPanelToGeneratorToolDataOnDisplay();
            List<Action> newActionsList = new ArrayList<>();
            Action pickupToolByToolAction = Action.newSingleArgAction(PICKUP_TOOL_BY_TOOL,
                    toolName);
    ...
    private XFuture<Boolean> switchTool(String toolName) {
        try {
            ...
            syncPanelToGeneratorToolDataOnDisplay();
            List<Action> newActionsList = new ArrayList<>();
            Action switchToolAction = Action.newSingleArgAction(
                    SWITCH_TOOL,
                    toolName);
    ...
    private XFutureVoid checkDbSupplierPublisherAsync() {
        if (null == this.crclGenerator) {
            ...
            ret.completeExceptionally(
                    new IllegalStateException("checkDbSupplierPublisher(null==pddlActionToCrclGenerator)"));
            return ret;
        }
        if (crclGenerator.isConnected()) {
            try {
                return XFutureVoid
                        .completedFutureWithName("checkDbSupplierPublisher.alreadyConnected." + getConnnectionURL());
            } catch (Exception ex) {
                LOGGER.log(Level.SEVERE, "", ex);
...
//    }
    ...
    private void removeOption(ExecutorOption key) {
        ...
        for (int i = 0; i < optionsCachedTable.getRowCount(); i++) {
            ...
            if (Objects.equals(keyCheck, key) || Objects.equals(keyCheck.toString(), key.toString())) {
...
//                optionsTableEventsCaused.incrementAndGet();
        ...
        if (key == ExecutorOption.ForBoolean.enableOptaPlanner || Objects.equals(key.toString(), ExecutorOption.ForBoolean.enableOptaPlanner.toString())) {
            if (val != null) {
                boolean newb = Boolean.valueOf(val.toString());
                if (newb != jCheckBoxEnableOptaPlanner.isSelected()) {
                    jCheckBoxEnableOptaPlanner.setSelected(newb);
                }
                if (newb != enableOptaplannerCachedCheckBox.isSelected()) {
...
//        }
    ...
    public Map<ExecutorOption, ?> getOptionsFromCachedTable() {
        ...
        for (int i = 0; i < optionsData.length; i++) {
            ...
            if (exOpt instanceof ExecutorOption.ForString) {
                ...
                throw new RuntimeException("invalid value of key=" + key + " (exOpt = " + exOpt + ")");
            }
            if (options.size() != i + 1) {
                System.out.println("ExecutorJPanel.getOptions() options.size()=" + options.size() + " key=" + key + ", i= " + i);
                logDebug("ExecutorJPanel.getOptions() options.size()=" + options.size() + " key=" + key + ", i= " + i);
                for (int j = 0; j < optionsData.length; j++) {
                    if (j == i) {
                        continue;
                    }
                    Object keyj = optionsData[j][0];
                    if (Objects.equals(key, keyj)) {
                        System.out.println("ExecutorJPanel.getOptions() Reduntant key=" + key + ", i= " + i + ", j=" + j);
                        logDebug("ExecutorJPanel.getOptions() Reduntant key=" + key + ", i= " + i + ", j=" + j);
                    }
                }
                if (null == options.get(key)) {
                    System.out.println("ExecutorJPanel.getOptions()  key=" + key + ", i= " + i + " not added to options map");
                    logDebug("ExecutorJPanel.getOptions() Reduntant key=" + key + ", i= " + i + " not added to options map");
                }
            }
        }
        if (options.size() != optionsData.length) {
            for (int i = 0; i < optionsData.length; i++) {
                Object key = optionsData[i][0];
                for (int j = i + 1; j < optionsData.length; j++) {
                    Object keyj = optionsData[j][0];
                    if (Objects.equals(key, keyj)) {
                        System.out.println("ExecutorJPanel.getOptions() Reduntant key=" + key + ", i= " + i + ", j=" + j);
                        logDebug("ExecutorJPanel.getOptions() Reduntant key=" + key + ", i= " + i + ", j=" + j);
    ...
    private javax.swing.JCheckBox jCheckBoxRequiredToolsEdit;
    ...
    String propsGetFileName(Properties props, String key) throws IOException {
        ...
        if (!f.exists() && (Utils.file(dir, newfilename.replace('\\', File.separatorChar))).exists()) {
            ...
            f = Utils.file(dir, newfilename);
        } else if (!f.exists() && (Utils.file(dir, newfilename.replace('/', File.separatorChar))).exists()) {
            newfilename = newfilename.replace('/', File.separatorChar);
            f = Utils.file(dir, newfilename);
        }
    ...
    private static final @Nullable String TEMP_DIR;
        ...
        try {
            if (null != propertiesFile && propertiesFile.exists()) {
                if (propertiesFile.isDirectory()) {
                    logDebug("Can not loadProperties file \"" + propertiesFile
                            + "\" : It is a directory instead of text file.");
                    return;
                }
                ...
                String saveProgramRunDataFlagProperty = props
                        .getProperty(ExecutorOption.ForBoolean.saveProgramRunData.name());
...
//            reloadActionsFile(reverseFlagFromProperty, true);
                ...
                String skuToSlotFilename = getShortSkuToEmptySlotPrefixMapCsvFileName();
                if (null != skuToSlotFilename && skuToSlotFilename.length() > 1) {
                    setOption(ExecutorOption.ForString.shortSkuToEmptySlotPrefixMapCsvFile, skuToSlotFilename);
                }
                initGenerateAbortLogFile();
    ...
    public void setOptionsTableValue(ExecutorOption opt, final Object propertyValue) {
        ...
        for (int i = 0; i < optionsCachedTable.getRowCount(); i++) {
            ...
            ExecutorOption optFromTable = (tableValue0 instanceof ExecutorOption)
...
//                    optionsTableEventsCaused.incrementAndGet();
        ...
        if (!foundit) {
            optionsCachedTable.addRow(new Object[]{opt.toString(), propertyValue});
        }
    }
...
//    private static final String SAVE_PROGRAM_RUN_DATA_FLAG = "saveProgramRunData";
    ...
    public @Nullable
    List<Action> reloadActionsFile(boolean newReverseFlag, boolean showInOptaplanner)
            throws IOException {
        setReverseFlag(newReverseFlag);
     ...
     */
    private void programLineListenerAccept(CrclSwingClientJPanel panel, int line, CRCLProgramType program,
        ...
        if (this.debug && null != program) {
            ...
            logDebug("sz = " + sz);
            logDebug("line = " + line);
            final CommandStatusType commandStatus = CRCLUtils.getNonNullCommandStatus(status);
    ...
    private volatile @Nullable List<PhysicalItem> availableToolHolders = null;
    ...
    public List<PhysicalItem> getToolsInHolders() {
        Map<String, PoseType> toolHolderPoseMap = crclGenerator.getToolHolderPoseMap();
        Map<String, String> toolHolderContentsMap = crclGenerator.getCurrentToolHolderContentsMap();
        List<PhysicalItem> newList = new ArrayList<>();
        for (Entry<String, PoseType> entry : toolHolderPoseMap.entrySet()) {
