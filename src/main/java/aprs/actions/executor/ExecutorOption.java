...
public interface ExecutorOption{
    ...
    public static <K extends Enum& ExecutorOption,V,P extends ExecutorOptionValuePair<K,V>>  Map<K,V> map(
            Class<K> keyClass, 
            Class<V> valueClass, 
            Class<P> pairClass, 
            ExecutorOptionValuePair<?,?> ... optionpairs) {
        Map<K, V> map = new EnumMap(keyClass);
        for (int i = 0; i < optionpairs.length; i++) {
            ExecutorOptionValuePair<?,?> optionpair = optionpairs[i];
            if (pairClass.isInstance(optionpair)) {
                P booloptionpair = pairClass.cast(optionpair);
